...
public class FaceGraphic extends GraphicOverlay.Graphic {
    ...
    public void draw(Canvas canvas) {
        ...
        if (face.getSmilingProbability() != null) {
            ...
            textWidth = Math.max(textWidth, idPaints[colorID].measureText(String.format(Locale.US,
                    "Happiness: %.2f", face.getSmilingProbability())));
        }

        if (face.getLeftEyeOpenProbability() != null) {
            yLabelOffset -= lineHeight;
            textWidth = Math.max(textWidth, idPaints[colorID].measureText(String.format(Locale.US,
                    "Left eye open: %.2f", face.getLeftEyeOpenProbability())));
        }

        if (face.getRightEyeOpenProbability() != null) {
            yLabelOffset -= lineHeight;
            textWidth = Math.max(textWidth, idPaints[colorID].measureText(String.format(Locale.US,
                    "Right eye open: %.2f", face.getRightEyeOpenProbability())));
        }

        // Draw labels
        canvas.drawRect(left - BOX_STROKE_WIDTH, top + yLabelOffset,
                left + textWidth + (2 * BOX_STROKE_WIDTH), top,
                labelPaints[colorID]);
        ...
        yLabelOffset = drawSmile(canvas, left, top, yLabelOffset, lineHeight, colorID);
        yLabelOffset = drawLeftEye(canvas, left, top, yLabelOffset, lineHeight, colorID);
        drawRightEye(canvas, left, top, yLabelOffset, colorID);
    ...
    private float drawSmile(Canvas canvas, float left, float top, float yLabelOffset, float lineHeight, int colorID) {
        if (face.getSmilingProbability() != null) {
            canvas.drawText(
    ...
    private float drawLeftEye(Canvas canvas, float left, float top, float yLabelOffset, float lineHeight, int colorID) {
        FaceLandmark leftEye = face.getLandmark(FaceLandmark.LEFT_EYE);
        if (face.getLeftEyeOpenProbability() != null) {
            canvas.drawText("Left eye open: " + String.format(Locale.US, "%.2f",
                    face.getLeftEyeOpenProbability()), left, top + yLabelOffset, idPaints[colorID]);
            yLabelOffset += lineHeight;
        }

        if (leftEye != null) {
            float leftEyeLeft = translateX(leftEye.getPosition().x) -
                    idPaints[colorID].measureText("Left Eye") / 2.0f;
            canvas.drawRect(leftEyeLeft - BOX_STROKE_WIDTH,
                    ...
                    labelPaints[colorID]);
            canvas.drawText("Left Eye", leftEyeLeft,
                    translateY(leftEye.getPosition().y) + ID_Y_OFFSET, idPaints[colorID]);
        }

        return yLabelOffset;
    }

    private void drawRightEye(Canvas canvas, float left, float top, float yLabelOffset, int colorID) {
        FaceLandmark rightEye = face.getLandmark(FaceLandmark.RIGHT_EYE);
        if (face.getRightEyeOpenProbability() != null) {
            canvas.drawText("Right eye open: " + String.format(Locale.US, "%.2f",
                    face.getRightEyeOpenProbability()), left, top + yLabelOffset,
                    idPaints[colorID]);
        }

        if (rightEye != null) {
            float rightEyeLeft = translateX(rightEye.getPosition().x) -
                    idPaints[colorID].measureText("Right Eye") / 2.0f;
            canvas.drawRect(rightEyeLeft - BOX_STROKE_WIDTH,
                    ...
                    labelPaints[colorID]);
            canvas.drawText("Right Eye", rightEyeLeft,
                    translateY(rightEye.getPosition().y) + ID_Y_OFFSET, idPaints[colorID]);
        }
    }
    ...
    private void drawFaceLandmark(Canvas canvas, @LandmarkType int landmarkType) {
        ...
        if (faceLandmark != null) {
            canvas.drawCircle(translateX(faceLandmark.getPosition().x),
