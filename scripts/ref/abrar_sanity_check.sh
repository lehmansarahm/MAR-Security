...
((IMG_TIME_DIFF=0))
((IMG_TIME_DIFF_LIMIT=5000)) # 5000ms, or 5sec
...
calculate_img_time_diff() {
    ...
    ((IMG_TIME_DIFF=${CURRENT_TIME}-${IMG_START_TIME}))
}
...
run_trial() {
    ...
    for IMG_FILE in $(cat ${IMG_FILE_LIST}); do
        ...
        IMG_START_TIME=$(date +%s%3)
        ((IMG_TIME_DIFF=0))

        while [ $IMG_TIME_DIFF -lt $IMG_TIME_DIFF_LIMIT ]; do
            print_resource_line ${IMG_FILE}
            calculate_img_time_diff ${IMG_START_TIME}
        done
